...
The `src/lib/api/` folder contains the shared API infrastructure including the Axios `client`, `queryClient`, and several utility functions specifically designed to work with `react-query`. Each feature contains its own `api.ts` file with feature-specific queries and mutations.
...
To get started, create an `api.ts` file within your feature folder (e.g., `src/features/feed/api.ts`). This file will hold all the feature's API logic including queries and mutations. You can apply this same pattern to any other features in your application.
...
1. Inside your feature folder (e.g., `src/features/feed/`), create or open the `api.ts` file.
...
Below is an excerpt from the feature's api.ts file showing the `usePosts` query:

<CodeBlock file="src/features/feed/api.ts" lang="tsx" meta="ins={1-13}" />
...
![use-query](https://github.com/ACT-House-Engineering/panama/assets/114411010/c052a0ee-8fba-436a-950f-a0c7e44cf3ae)

:::

Now that we have created our custom hook, we can use it in our screen to display a list of posts. Follow the steps below to achieve this:

1. Create a new screen in your feature folder (e.g., `src/features/feed/feed-screen.tsx`).
...
<CodeBlock file="src/features/feed/feed-screen.tsx" lang="tsx" meta="ins={1-30}" />
...
Add the `usePost` query to your feature's `api.ts` file alongside the `usePosts` hook:

<CodeBlock file="src/features/feed/api.ts" lang="tsx" meta="ins={15-23}" />

Now our hook is ready to be used in our post details screen:

<CodeBlock file="src/features/feed/post-detail-screen.tsx" lang="tsx" meta="ins={1-20}" />
...
1. Open your feature's `api.ts` file (e.g., `src/features/feed/api.ts`).
...
Add the `useAddPost` mutation to your feature's `api.ts` file:

<CodeBlock file="src/features/feed/api.ts" lang="tsx" meta="ins={25-35}" />
...
Now that we have our mutation hook ready. Let's create a new screen called `AddPostScreen` and use data from `useAddPost` hook to create a new post:
...
<CodeBlock file="src/features/feed/add-post-screen.tsx" />
...
![use-mutation](https://github.com/ACT-House-Engineering/panama/assets/11137944/c322f827-b71d-4629-a337-eb7cd96d4125)
...
![React Query plugin web interface](https://github.com/ACT-House-Engineering/panama/assets/114411010/26bc96ef-e6cb-49c3-be3c-006d6901e440)
